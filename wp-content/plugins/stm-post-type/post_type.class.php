...
class STM_PostType {
	...
	public static function registerPostType( $postType, $title, $args ) {
		...
		$labels      = array(
			...
			'add_new'            => __( 'Agregar Nuevo', STM_POST_TYPE ),
			'add_new_item'       => __( 'Agregar Nuevo ' . $title, STM_POST_TYPE ),
			'edit_item'          => __( 'Editar ' . $title, STM_POST_TYPE ),
			'new_item'           => __( 'Nuevo ' . $title, STM_POST_TYPE ),
			'all_items'          => __( 'Todo ' . $pluralTitle, STM_POST_TYPE ),
			'view_item'          => __( 'Ver ' . $title, STM_POST_TYPE ),
			'search_items'       => __( 'Buscar ' . $pluralTitle, STM_POST_TYPE ),
			'not_found'          => __( 'No ' . $pluralTitle . ' encontrado', STM_POST_TYPE ),
			'not_found_in_trash' => __( 'No ' . $pluralTitle . '  encontrado en la basura', STM_POST_TYPE ),
			'parent_item_colon'  => '',
		...
		foreach ( $fields as $key => $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'hidden':
					...
					break;
				case 'file':
					$file = __("Archivo no elegido", STM_POST_TYPE);
					if ($meta) {
					...
					echo '
						<div class="stm_metabox_image stm_metabox_file">
							...
							<input class="stm_upload_image upload_button_'. $key .' button-primary" type="button" value="' . __( 'Elegir PDF', STM_POST_TYPE ). '" />
						...
						<script type="text/javascript">
							jQuery(function($) {
								...
								$(".stm_remove_image").click(function(){
									...
									$(this).closest(".stm_metabox_image").find(".custom_preview_file").text("'.__("Ningun archivo elegido", STM_POST_TYPE).'");
									return false;
								});
							});
						</script>
				...
				case 'images':
					...
					echo '</div>'.
						 '<div class="stm-add-media button button-primary">'.__( "Agregar", STM_POST_TYPE ).'</div>'.
						 '<script>
						    jQuery(function($) {
