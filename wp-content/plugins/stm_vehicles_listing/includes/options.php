...
function stm_vehicle_listing_settings_page() {
	...
	if ( ! empty( $_POST['stm_vehicle_listing_options'] ) ) {
		...
		if ( $getted_post_array['single_name'] != '' and $getted_post_array['plural_name'] != '' and $getted_post_array['slug'] != ''  ) {
			...
			if(!empty($getted_post_array['numeric']) and $getted_post_array['numeric'] == 'on') {
				...
				//Check if Slider
				if(!empty($getted_post_array['slider']) and $getted_post_array['slider'] == 'on') {
					$edited_post['slider'] = true;
				} else {
					$edited_post['slider'] = false;
				}
			} else {
			...
			if(!empty($getted_post_array['use_on_tabs']) and $getted_post_array['use_on_tabs'] == 'on') {
				$edited_post['use_on_tabs'] = true;
			} else {
				$edited_post['use_on_tabs'] = false;
			}
	...
	if(isset($_GET['delete'])){
		...
		<script type="text/javascript">
			jQuery(document).ready(function(){
				window.history.pushState('','',"<?php echo get_site_url().remove_query_arg('delete', false); ?>");
			})
		</script>
		<?php
	}
	...
	if ( ! empty( $_POST['stm_listing_edit'] ) ) {
		...
		if ( !empty($edit_exist_item['new_single_name']) and $edit_exist_item['new_single_name'] != '' and $edit_exist_item['new_plural_name'] != '' and $edit_exist_item['old_slug'] != '' ) {
			$edit_exist = array(
			...
			if(!empty($edit_exist_item['edit_numeric']) and $edit_exist_item['edit_numeric'] == 'on') {
				...
				if(!empty($edit_exist_item['edit_slider']) and $edit_exist_item['edit_slider'] == 'on') {
					$edit_exist['slider'] = true;
				} else {
					$edit_exist['slider'] = false;
				}
			} else {
			...
			if(!empty($edit_exist_item['use_on_tabs']) and $edit_exist_item['use_on_tabs'] == 'on') {
				$edit_exist['use_on_tabs'] = true;
			} else {
				$edit_exist['use_on_tabs'] = false;
			}
	...
	<div id="col-container">
		...
		<div id="col-right">
			<div class="col-wrap">
				<form method="post" id="stm_posts_filter" action="<?php echo get_site_url(); ?>/wp-admin/edit.php?post_type=listings&page=listing_categories">
							...
							foreach ( $options as $option_key => $option ): ?>
								...
								<tr id="<?php echo esc_attr($option_key); ?>">
									<?php foreach ( $option as $single_option_key => $single_option ): ?>
										...
										<?php elseif($single_option_key == 'use_on_single_listing_page' or $single_option_key =='use_on_car_listing_page' or $single_option_key =='use_on_car_archive_listing_page' or $single_option_key =='use_on_single_car_page' or $single_option_key =='use_on_car_filter' or $single_option_key =='use_on_tabs' or $single_option_key =='use_on_car_modern_filter'  or $single_option_key =='use_on_car_modern_filter_view_images' or $single_option_key =='use_on_car_filter_links' or $single_option_key == 'use_on_directory_filter_title' or $single_option_key == 'number_field_affix' or $single_option_key == 'listing_rows_numbers' or $single_option_key == 'enable_checkbox_button' or $single_option_key == 'use_in_footer_search' or $single_option_key == 'listing_taxonomy_parent' or $single_option_key == 'font' or $single_option_key == 'slider'): ?>
									...
									<?php endforeach; ?>
									<td>
										<a href="<?php echo get_site_url(); ?>/wp-admin/edit-tags.php?taxonomy=<?php echo esc_attr($option['slug']); ?>&post_type=listings">
									...
									<td>
										<a
											...
											data-slider="<?php echo esc_attr($option['slider']) ?>"
											...
											data-use-on-tabs="<?php echo esc_attr($option['use_on_tabs']) ?>"
											...
											<?php if(!empty($option['number_field_affix'])): ?>
												data-number-field-affix="<?php echo $option['number_field_affix']; ?>"
											<?php endif; ?>
				...
				<div class="export_settings">
					<h3><?php esc_html_e('Export/Import Listing settings', STM_VEHICLE_LISTING); ?></h3>
					<a href="<?php echo esc_url( add_query_arg( 'export_settings', '1', 'edit.php?post_type=listings&amp;page=listing_categories' ) ); ?>" class="button button-primary">
						<?php esc_html_e('Get my settings', STM_VEHICLE_LISTING); ?>
					</a>
					...
					<form method="post" enctype="multipart/form-data">
						...
						<input type="submit" class="button button-primary" value="<?php esc_html_e('Set my settings'); ?>">
					</form>
				</div>
			</div>
		</div>
		<div id="col-left">
			<div class="col-wrap">
				...
				<div class="form-wrap stm_edit_item_wrap">
					<form class="" action="<?php echo get_site_url(); ?>/wp-admin/edit.php?post_type=listings&page=listing_categories" method="post">
						...
						<div class="form-field">
							<label><?php _e( 'Display as Slider field', STM_VEHICLE_LISTING ); ?></label>
							<input
								type="checkbox"
								name="stm_listing_edit[edit_slider]"
								id="stm_edit_item_slider"
								/>
							<p><i><?php _e( 'Only for number field', STM_VEHICLE_LISTING ); ?></i></p>
						</div>
						...
						<div class="form-field">
							<label><?php _e( 'Use on Archive page and Single Motorcycle page(tabs)', STM_VEHICLE_LISTING ); ?></label>
							<input
								type="checkbox"
								name="stm_listing_edit[use_on_tabs]"
								id="use_on_tabs"
								/>
						</div>
						<div class="form-field">
				...
				<div class="form-wrap stm-new-filter-category">
					<form method="post" action="<?php echo get_site_url(); ?>/wp-admin/edit.php?post_type=listings&page=listing_categories">
						<div class="form-field">
						...
						<div class="form-field">
							<label><?php _e( 'Display field as slider', STM_VEHICLE_LISTING ); ?></label>
							<input
								type="checkbox"
								name="stm_vehicle_listing_options[slider]"
								/>
							<p><i><?php _e( 'Only for number field', STM_VEHICLE_LISTING ); ?></i></p>
						</div>
						<div class="form-field hidden">
						...
						<div class="form-field stm-modern-filter-unit">
							<label><?php _e( 'Use in Archive Page and Single Motorcycle Page (tabs)', STM_VEHICLE_LISTING ); ?></label>
							<input
								type="checkbox"
								name="stm_vehicle_listing_options[use_on_tabs]"
								/>
						</div>
						<div class="form-field">
							<label><?php _e( 'Use on car modern filter', STM_VEHICLE_LISTING ); ?></label>
...
function stm_get_cat_icons($font_pack = 'fa') {
	...
	if($font_pack == 'fa') {
		...
		$fonts_custom_type_2 = json_decode( file_get_contents( get_template_directory() . '/assets/js/boat_icons.json' ), true );
		...
		$fonts_custom_type_2 = json_decode( file_get_contents( get_template_directory() . '/assets/js/moto_icons.json' ), true );
...
function stm_get_custom_taxonomy_count($tax_term, $taxonomy_name) {
	...
	if(!empty($tax_term) and !empty($taxonomy_name)) {
...
function stm_export_settings() {
	...
	if(!empty($_FILES) and !empty($_FILES['import_settings']) and !empty($_FILES['import_settings']['tmp_name'])) {
