...
function stm_ajax_filter() {
	...
	$response['posts_per_page_link']  = $stm_listing_filter['posts_per_page_link'];
	...
	if(!empty($stm_listing_filter['featured'])) {
		if ( $stm_listing_filter['featured']->have_posts() ):
			ob_start();
			while ( $stm_listing_filter['featured']->have_posts() ) {
				$stm_listing_filter['featured']->the_post();
			...
			$response['featured_html'] = ob_get_contents();
			ob_end_clean();
		endif;
	}
	...
	if($show_pagination) {
		...
		$response['pagination'] .= '<div class="clearfix">';
		...
		if ( stm_is_motorcycle() ) {
			$response['pagination'] .= '<div class="stm-motorcycle-per-page stm_boats_view_by">';
			ob_start();
			get_template_part( 'partials/listing-layout-parts/items-per', 'page' );
		...
		$response['pagination'] .= '<div class="stm-motorcycle-per-page stm_boats_view_by left-mode">';
		ob_start();
		get_template_part( 'partials/listing-layout-parts/items-per', 'page' );
		$response['pagination'] .= ob_get_contents();
		ob_end_clean();
...
function stm_custom_register() {
	...
	if(empty($errors)) {
		...
		if ( ! is_wp_error( $user_id ) ) {
			...
			/*Mail*/
			$to      = get_bloginfo( 'admin_email' );
			$subject = esc_html__( 'New User', 'motors' );
			$body    = esc_html__( 'New user Registered. Nickname: ', 'motors' ) . $user_login;
...
if( !function_exists('stm_ajax_add_a_car_media')) {
	function stm_ajax_add_a_car_media() {
		if ( stm_is_site_demo_mode() ) {
			wp_send_json( array( 'message' => esc_html__( 'Site is on demo mode', 'motors' ) ) );
			exit;
		}
		...
		if ( ! $post_id ) {
			...
			wp_send_json( array( 'message' => esc_html__( 'Some error occurred, try again later', 'motors' ) ) );
			exit;
		}
		...
		if ( intval( get_post_meta( $post_id, 'stm_car_user', true ) ) != intval( $user_id ) ) {
			...
			wp_send_json( array( 'message' => esc_html__( 'You are trying to add car to another car user, or your session has expired, please sign in first', 'motors' ) ) );
			exit;
		}
		...
		if ( ! empty( $_FILES ) ) {
			...
			if ( count( $_FILES['files']['name'] ) > $max_uploads ) {
				$error               = true;
				$response['message'] = sprintf( esc_html__( 'Sorry, you can upload only %d images per add', 'motors' ), $max_uploads );
			} else {
		...
		foreach ( $files_approved as $f => $name ) {
			$new_filename = 'post_id_' . $post_id . '_' . stm_media_random_affix() . '.' . pathinfo( $name, PATHINFO_EXTENSION );
		...
		if ( $updating ) {
			...
			$to      = get_bloginfo( 'admin_email' );
			$subject = esc_html__( 'Car Updated', 'motors' );
			$body    = esc_html__( 'User Updated car. User id - ', 'motors' ) . $user_id . '. ';
			$body   .= esc_html__( 'Car ID - ', 'motors' ) . $post_id;

		} else {
			$response['message'] = esc_html__( 'Car added, redirecting to your account profile', 'motors' );

			$to      = get_bloginfo( 'admin_email' );
			$subject = esc_html__( 'Car Added', 'motors' );
			$body    = esc_html__( 'User Added car. User id - ', 'motors' ) . $user_id . $user_id . '. ';;
			$body   .= esc_html__( 'Car ID - ', 'motors' ) . $post_id;
		}
